...
from flask import (
    Blueprint, render_template, current_app, send_from_directory, redirect, url_for, flash, request)
...
def detect(image_id):
    user_image = db.session.query(UserImage).filter(
        # bring the record from user_images table based on image_id
        UserImage.id == image_id).first()
    if user_image is None:
        flash("No image to detect")
    ...
    try:
        ...
        flash("There was an error in detection processes")
        db.sesison.rollback()
...
def upload_image():
    ...
    if form.validate_on_submit():
        ...
        # change image file name to uuid for security reasons
